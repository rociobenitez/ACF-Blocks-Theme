<?php
/**
 * Componente reutilizable: card de servicio.
 * Usa $args: ['post_id' => int, 'cta_text' => string, 'extra_cls' => string]
 */
...
if ( ! $post_id ) return; // FALTA_DATO:id

$title     = get_the_title($post_id);
if ( ! $title ) return; // FALTA_DATO:title (ya filtrado antes, doble seguro)

$permalink = get_permalink($post_id);
$excerpt   = wp_trim_words( wp_strip_all_tags( get_the_excerpt($post_id) ), 28, '…' );

// Thumbnail (con fallback accesible)
	...
	<a class="card-link" href="<?php echo esc_url($permalink); ?>" aria-label="<?php echo esc_attr( $title ); ?>">
		...
		<div class="card-content">
			...
			<div class="card-actions">
				<span class="btn btn-outline-primary btn--sm card-btn">
